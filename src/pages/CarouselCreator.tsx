...
export default function CarouselCreator() {
  ...
  // Handle initial data from location state
  useEffect(() => {
    if (location.state?.initialData && !carousel) {
      setCarousel(location.state.initialData);
      if (location.state.metadata) {
        const { theme, postType, offer } = location.state.metadata;
        if (theme) setTopic(theme);
        if (postType) setPostType(postType);
        if (offer) setCustomInstructions(`Oferta/CTA: ${offer}`);
      }
      
      // Auto-generate designs if requested
      if (location.state.autoGenerateDesign) {
  ...
  }, [location.state, carousel, setCarousel]);
