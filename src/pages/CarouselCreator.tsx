...
import {
  ...
  Calendar, Palette, Type, Images, Download, ArrowRight, Lightbulb, Target, BookOpenCheck, X
} from "lucide-react";
...
export default function CarouselCreator() {
  ...
  const state = location.state as { initialData?: CarouselData; metadata?: any; autoGenerateDesign?: boolean; topic?: string; preselectedFormat?: string; researchSource?: ResearchSourceContext } | null;
  ...
  const [sourceContext, setSourceContext] = useState<ResearchSourceContext | null>(null);
  const navigationStateLoaded = useRef(false);
  ...
  useEffect(() => {
    if (navigationStateLoaded.current) return;
    navigationStateLoaded.current = true;
    ...
    if (state?.researchSource) setSourceContext(state.researchSource);
  ...
  const handleSave = async () => {
    ...
    const result = await saveGeneration({
      ...
      input_data: { topic, postType, contentPillar, source: sourceContext } as unknown as Json,
      favorito: false,
        ...
        <div className="container flex items-center justify-between h-14 px-4">
          ...
          {carousel && (
            <div className="flex gap-2">
              ...
              {!editingWeekItem && (
                <Button variant="outline" size="sm" onClick={handleNewContent}>
      ...
      <main className="container px-4 py-6">
        ...
        {weekContent.length > 0 && !carousel && !editingWeekItem ? (
          ...
          <div className="max-w-4xl mx-auto pt-6 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {sourceContext && (
              <div className="relative overflow-hidden rounded-2xl border border-primary/20 bg-primary/[0.04] p-4 pr-12">
                <div className="flex items-start gap-3">
                  <div className="mt-0.5 rounded-xl bg-primary/10 p-2 text-primary">
                    <BookOpenCheck className="h-4 w-4" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs font-semibold text-primary">Conteúdo ancorado em fonte real</p>
                    <p className="mt-1 line-clamp-2 text-sm font-medium leading-5">{sourceContext.title}</p>
                    <p className="mt-1 text-xs text-muted-foreground">
                      {sourceContext.source} · a IA usará apenas os fatos presentes no resumo fornecido
                    </p>
                  </div>
                </div>
                <Button
